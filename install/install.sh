...
step_check_system() {
    ...
    case "$OS_NAME" in
        ubuntu|debian)
            ...
            apt-get update -y
            ;;
        centos|rhel|rocky|almalinux|openEuler|kylin|anolis|NingOS|uos|opencloudos)
            PKG_MANAGER="dnf"
    ...
    if command -v python3 &>/dev/null; then
        ...
        if [ "$py_major" -gt 3 ] || { [ "$py_major" -eq 3 ] && [ "$py_minor" -ge 13 ]; }; then
            log_warn "Python 版本 ($py_version) 过高，面板最高支持 Python 3.12"
            echo ""
            echo -ne "是否编译安装 Python 到 /opt? ${YELLOW}[y/n]${NC}: "
            read -r py_choice
            case "$py_choice" in
                y|Y)
                    echo -ne "选择要编译的 Python 版本 (1: 3.10, 2: 3.11, 3: 3.12) ${YELLOW}[默认 3]${NC}: "
                    ...
                    ;;
                n|N)
                    log_error "已取消安装，需要 Python 3.10-3.12 才能运行面板"
                    exit 1
                    ;;
                *)
                    log_error "无效输入"
