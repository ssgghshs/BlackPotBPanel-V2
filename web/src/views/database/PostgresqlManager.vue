<template>
  <a-card class="pgsql-manager-card">
    <template #title>
    ...
    <a-drawer
      ...
      :title="t('editUser') + ' - ' + editUserForm.username"
      :width="500"
      :footer="true"
      @cancel="cancelEditUser"
    >
      <a-form layout="vertical" :model="editUserForm">
        <a-form-item :label="t('newUsername')">
...
const editUserForm = ref({ username: '', new_username: '', new_password: '' });
...
const userColumns = [
  { title: t.value('username'), dataIndex: 'username', width: 120 },
  { title: t.value('superuser'), dataIndex: 'is_superuser', width: 100,
    render: ({ record }) => record.is_superuser ? t.value('yes') : t.value('no')
  },
  { title: t.value('canCreateDb'), dataIndex: 'can_create_db', width: 100,
    render: ({ record }) => record.can_create_db ? t.value('yes') : t.value('no')
  },
...
const goBack = () => {
  router.push('/database/postgresql');
};

const fetchServerInfo = async () => {
  try {
    const res = await getPgsqlServers({ skip: 0, limit: 1000 });
...
const fetchDatabases = async () => {
  ...
  try {
    ...
    const res = await getPgsqlDatabases(serverId.value);
...
const fetchTables = async () => {
  ...
  try {
    ...
    const res = await getPgsqlTables(serverId.value, selectedDatabase.value);
...
const fetchTabData = async (tableName) => {
  ...
  try {
    ...
    const res = await getPgsqlTableData(serverId.value, selectedDatabase.value, tableName, params);
    if (res.error) {
...
const handleExecuteSql = async () => {
  ...
  try {
    ...
    const res = await executePgsqlSql(serverId.value, selectedDatabase.value, { sql });
    sqlTab.sqlResult = {
      success: res.success,
      message: res.message,
...
const handleQuerySql = async () => {
  ...
  try {
    sqlTab.sqlLoading = true;
    const res = await queryPgsqlSql(serverId.value, selectedDatabase.value, { sql });
    sqlTab.sqlResult = {
      success: res.success,
      message: res.message,
      columns: res.columns || null,
...
const openAddDataDrawer = async (tableName) => {
  ...
  try {
    const res = await getPgsqlTableColumns(serverId.value, selectedDatabase.value, tableName);
...
const handleAddData = async () => {
  try {
    ...
    await addPgsqlData(serverId.value, selectedDatabase.value, {
...
const handleEditData = async () => {
  try {
    ...
    await updatePgsqlData(serverId.value, selectedDatabase.value, {
...
const handleDeleteData = async () => {
  try {
    ...
    await deletePgsqlData(serverId.value, selectedDatabase.value, {
...
const deleteTableOperation = async () => {
  ...
  Modal.confirm({
    ...
    async onOk() {
      try {
        await deletePgsqlTable(serverId.value, selectedDatabase.value, tableName);
...
const handleCreateDb = async () => {
  ...
  try {
    ...
    await createPgsqlDatabase(serverId.value, {
...
const confirmDeleteDatabase = () => {
  ...
  Modal.confirm({
    ...
    async onOk() {
      try {
        await deletePgsqlDatabase(serverId.value, selectedDatabase.value);
...
const handleCreateTable = async () => {
  ...
  try {
    ...
    const res = await createPgsqlTable(serverId.value, selectedDatabase.value, { sql: createTableForm.value.sql.trim() });
    if (res.success) {
...
const fetchUsers = async () => {
  try {
    ...
    const res = await getPgsqlUsers(serverId.value);
...
const handleCreateUser = async () => {
  ...
  try {
    ...
    await createPgsqlUser(serverId.value, {
...
const confirmDeleteUser = (record) => {
  Modal.confirm({
    title: t.value('confirmDelete'),
    content: `${t.value('confirmDeleteUser')} "${record.username}"?`,
    okText: t.value('confirm'),
    cancelText: t.value('cancel'),
    okType: 'danger',
    async onOk() {
      try {
        await deletePgsqlUser(serverId.value, { username: record.username });
...
const handleGrantUser = async () => {
  ...
  try {
    ...
    await grantPgsqlPrivileges(serverId.value, {
...
const cancelEditUser = () => {
  ...
  editUserForm.value = { username: '', new_username: '', new_password: '' };
};

const handleEditUser = async () => {
  if (!editUserForm.value.new_username && !editUserForm.value.new_password) {
  ...
  try {
    ...
    await alterPgsqlUser(serverId.value, payload);
