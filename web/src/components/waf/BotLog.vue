<template>
  <div class="bot-log-container">
    ...
    >
      ...
      <template #verify_mode="{ record }">
        <a-tag :color="getVerifyModeColor(record.verify_mode)">
          {{ getVerifyModeText(record.verify_mode) }}
        </a-tag>
      </template>
      <template #verification_status="{ record }">
        <a-tag :color="getVerificationStatusColor(record.verification_status)">
          {{ getVerificationStatusText(record.verification_status) }}
        </a-tag>
      </template>
      <template #application_url="{ record }">
        <a-tooltip position="top" :content="record.application_url">
          <span class="application-url">{{ record.application_url }}</span>
        </a-tooltip>
      </template>
    ...
    >
      <div v-if="selectedLog" class="log-detail">
        <a-descriptions :column="1" bordered>
          ...
          <a-descriptions-item :label="t('verifyMode')">
            <a-tag :color="getVerifyModeColor(selectedLog.verify_mode)">
              {{ getVerifyModeText(selectedLog.verify_mode) }}
            </a-tag>
          </a-descriptions-item>
          <a-descriptions-item :label="t('verificationStatus')">
            <a-tag :color="getVerificationStatusColor(selectedLog.verification_status)">
              {{ getVerificationStatusText(selectedLog.verification_status) }}
            </a-tag>
          </a-descriptions-item>
    ...
    >
      <p>{{ t('clearBotLogsConfirmMessage') }}</p>
    </a-modal>
  </div>
</template>
...
import { getBotLog, cleanBotLog } from '../../api/waf';
...
const columns = computed(() => [
  ...
  {
    title: t.value('verifyMode'),
    dataIndex: 'verify_mode',
    slotName: 'verify_mode',
    width: 120
  },
  {
    title: t.value('verificationStatus'),
    dataIndex: 'verification_status',
    slotName: 'verification_status',
    width: 120
  },
  {
    title: t.value('action'),
...
const getActionText = (action) => {
  const textMap = {
    'verified': t.value('verified'),
    'challenge': t.value('challenge')
  };
  return textMap[action] || action;
};

const getVerifyModeColor = (mode) => {
  const colorMap = {
    '0': 'blue',
    '1': 'purple',
    '2': 'orange'
  };
  return colorMap[mode] || 'gray';
};

const getVerifyModeText = (mode) => {
  const textMap = {
    '0': t.value('Seamlesserification'),
    '1': t.value('5SecondVerification'),
    '2': t.value('SlidingVerification')
  };
...
const getVerificationStatusText = (status) => {
  const textMap = {
    'triggered': t.value('triggered'),
    'passed': t.value('passed'),
...
const fetchLogs = async (page = 1) => {
  loading.value = true;
  try {
    const response = await getBotLog({
      skip: 0,
      limit: 10000
    });
    ...
    console.error('获取BOT验证日志失败:', error);
...
const handleClearLogs = async () => {
  cleaning.value = true;
  try {
    const response = await cleanBotLog({});
    ...
    console.error('清理BOT验证日志失败:', error);
